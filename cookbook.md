...
The pipeline: Manully clean the data -> Read data -> Process and clean data -> training data
...
Now we are ready for training. Three models can be chosen in the script (naive Bayes, k nearest neighbor, support vector machine). We tested and tuned all of them and showed the analysis as following. The parameters used here to (![Binary classification labeled](/img/Binaryclassificationlabeled.png)
...
Thus, for small trainning data set () NB is a good model giving a acceptable result with sensitivity at 75%, precision around 43% and f1 score around 53%.
...
3. SVM model
Here we choose the c-classification SVM model with RBL kenel
Parameters for RBL kenel are cost, gamma, sparse level and size of the trainning data set. 